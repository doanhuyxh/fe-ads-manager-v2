...
export async function POST(req: NextRequest) {
    try {
        ...
        const { _id, time, startDate, keywords, accountId, chatZaloId, status } = body;
        if (!time || !startDate || !keywords || !chatZaloId || !accountId) {
            return errorResponse("vui lòng điền đủ thông tin");
        }
        await connectToDB();
        let schedule;
        if (_id) {
            const updateData: any = { time, startDate, keywords, accountId, chatZaloId, status };
            
            schedule = await Schedule.findByIdAndUpdate(_id, updateData, { new: true });
            return successResponse(schedule, "Cập nhật thành công");
        } else {
            schedule = await Schedule.create({ time, startDate, keywords, accountId, chatZaloId, status });
